...
internal object SourceCodeErasingUnixPrinter : SourceCodePrinter {
    /**
    ...
    override fun logOneLine(source: SourceCodeLogger, level: LogLevel, logger: Logger): String {
        ...
        // Upper line.
        val result = logUpperLine(source, level, logger)
        sb.append(result)
        ...
        if (source.message.isNullOrBlank()) {
            ...
            if (!source.messageAtBottom) {
                ...
                when {
                    source.message.isNullOrBlank() -> sb.append(level.color.boldAndColorText(lineNumStr))
                    else -> {
                        sb.append(level.color.boldAndColorText(lineNumStr))
                        sb.append(' ')
                        sb.append(AnsiColor.boldText(indentText(source.message!!, fromIndex.column + 4 + length)))
                    }
                }
                ...
                sb.append(level.color.boldAndColorText("└─>"))
                sb.append("  ")
                sb.append(AnsiColor.boldText(indentText(source.message!!, 2)))
                sb.append('\n')
            }
        }
    ...
    override fun logMore10Multiline(source: SourceCodeLogger, level: LogLevel, logger: Logger): String {
        ...
        for (line in lineSequence) {
            ...
            if (currentRow == toIndex.row) {
                ...
                sb.append(level.color.boldAndColorText("·".repeat(index - 1)))
                sb.append('\n')
            } else {
